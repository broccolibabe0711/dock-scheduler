...
Numbered so they can be cited. Each one is visible in the product, not only here.
...
7. A vessel's length comes only from the registry sheets; when two registry rows disagree, the vessel has **no** length until a person chooses.
...
18. One coordinator uses the system; there is no authentication or concurrent editing in v1.
